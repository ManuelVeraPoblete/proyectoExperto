...
import PostulantesSheet from '@/components/client/PostulantesSheet';
...
export default function MisTrabajosClienteScreen() {
  ...
  // Sheet postulantes
  const [postulantesJob, setPostulantesJob] = useState<ClientJob | null>(null);
  ...
  const renderJob = ({ item }: { item: ClientJob }) => {
    const hasProposals = (item.proposalCount ?? 0) > 0;
    ...
    return (
      <View style={styles.card}>
        ...
        {isActive && hasProposals && (
          <TouchableOpacity
            style={styles.postulantesBtn}
            onPress={() => setPostulantesJob(item)}
          >
            <MaterialCommunityIcons name="account-multiple" size={15} color={COLORS.secondary} />
            <Text style={styles.postulantesBtnText}>
              Ver postulantes ({item.proposalCount})
            </Text>
            <MaterialCommunityIcons name="chevron-right" size={15} color={COLORS.secondary} />
          </TouchableOpacity>
        )}

        {/* Sin postulantes aún */}
        {isActive && !hasProposals && (
          <View style={styles.noProposalsRow}>
            <MaterialCommunityIcons name="clock-outline" size={13} color={COLORS.textSecondary} />
            <Text style={styles.noProposalsText}>Esperando postulantes...</Text>
          </View>
        )}
  ...
  return (
    <View style={styles.container}>
      ...
      {/* ── Sheet postulantes ── */}
      <PostulantesSheet
        visible={postulantesJob !== null}
        jobId={postulantesJob?.id ?? ''}
        jobTitle={postulantesJob?.title ?? ''}
        onClose={() => setPostulantesJob(null)}
        onAssigned={() => {
          setPostulantesJob(null);
          refreshJobs();
        }}
      />
      ...
      <Modal
        ...
        transparent
        animationType="slide"
        onRequestClose={() => setCloseModal(null)}
      >
          ...
          >
            ...
            >
              <Text style={styles.buttonText}>
                {isClosingJob ? 'Cerrando...' : 'Confirmar Cierre'}
              </Text>
            </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  postulantesBtn: {
    ...
    backgroundColor: `${COLORS.secondary}08`, marginBottom: 4,
  },
  postulantesBtnText: { flex: 1, fontSize: 13, color: COLORS.secondary, fontWeight: '600' },

  noProposalsRow: {
    flexDirection: 'row', alignItems: 'center', gap: 5, marginBottom: 4,
  },
  noProposalsText: { fontSize: 12, color: COLORS.textSecondary },
